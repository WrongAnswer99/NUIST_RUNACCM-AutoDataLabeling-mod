...
点击"导出当前样本"时，自动先同步主摄像机的视角（含 Y 轴锁定逻辑），再执行导出。避免导出时相机参数与当前视角不一致。

### 涉及文件

| 文件 | 变更 |
|------|------|
| `js/ui.js` | 导出按钮事件改为先调用 `syncMainViewToExport()` 再导出 |
...
| `js/ui.js` | +123/-4 | 天空盒事件 + Y 轴锁定 + 坐标输入修复 + 导出命名重构 + 导出自动同步视角 |
...
> 以上在源代码基础上新增或修改的代码均由 DeepSeek 辅助编写。
